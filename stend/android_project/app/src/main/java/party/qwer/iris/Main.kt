    ...
    companion object {
        ...
        fun main(args: Array<String>) {
            try {
                ...
                // [Stend] Integrated FriendObserver (Derived from DBManager_deploy analysis)
        // Integrated Advanced Stend Observer
        val advancedObserver = AdvancedObserver(kakaoDb, wsEventFlow)
        advancedObserver.installTriggers()
        Thread {
            while (true) {
               try {
                   advancedObserver.pollEvents()
                   Thread.sleep(2000)
               } catch (e: Exception) {
                   e.printStackTrace()
               }
        ...
        println("AdvancedObserver started")
